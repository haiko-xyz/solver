...
fn test_change_oracle() {
    ...
    rev_solver.change_oracle(new_oracle);

    // Get oracle and run check.
    let oracle = rev_solver.oracle();
        ...
        .assert_emitted(
            @array![
                (
                    ...
                    ReversionSolver::Event::ChangeOracle(
                        ReversionSolver::ChangeOracle { oracle: new_oracle }
                    )
                )
            ]
        );
...
fn test_change_oracle_unchanged() {
    ...
    start_prank(CheatTarget::One(solver.contract_address), owner());
    let oracle = rev_solver.oracle();
    rev_solver.change_oracle(oracle);
}
