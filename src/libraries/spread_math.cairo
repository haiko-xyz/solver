...
use haiko_solver_replicating::types::{replicating::{MarketState, MarketParams, PositionInfo}, solver::SwapParams}; 
...
pub fn get_virtual_position(
    ...
    let (lower_sqrt_price, upper_sqrt_price) = if is_bid {
        (price_math::limit_to_sqrt_price(limit - market_params.range, 1), price_math::limit_to_sqrt_price(limit, 1))
    } else {
        (price_math::limit_to_sqrt_price(limit, 1), price_math::limit_to_sqrt_price(limit + market_params.range, 1))
    };
...
pub fn get_delta(max_delta: u32, base_reserves: u256, quote_reserves: u256, price: u256) -> i32 {
    ...
    let spread: u32 = math::mul_div(max_delta.into(), skew, DENOMINATOR, false)
        .try_into()
        .unwrap();
